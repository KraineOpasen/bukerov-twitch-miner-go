...
from .. import analyze, ancestry, cli, config, ghadapter, states
...
class TestCheckAndPrepareAgree(WiringCase):
    ...
    def test_equal_pin_with_default_ref_drift_is_blocked_without_fetch(self):
        scenario = self.scenario()
        self.patch_resolve(scenario, scenario.base_sha, "trunk")
        with tempfile.TemporaryDirectory() as workdir:
            result = cli._analyze_one(scenario.provider, scenario.root, workdir)
        self.assertEqual(scenario.base_sha, result.target_sha)
        self.assertFalse(result.drifted)
        self.assertTrue(result.is_blocked)
        self.assertEqual(states.BLOCKED, result.state)
        self.assertEqual([analyze.ANCESTRY], [reason.code for reason in result.blocked])

    def test_monitor_only_benign_advance_is_terminal_and_keeps_exact_target(self):
        """A quiet donor monitor result may not escape as production DRIFT_DETECTED."""

        root = os.path.dirname(os.path.dirname(os.path.dirname(
            os.path.dirname(os.path.abspath(__file__)))))
        provider = next(item for item in config.load(root) if item.monitor_only)
        target = "f" * 40

        class UnchangedWatchRepo:
            @staticmethod
            def commit_exists(_sha):
                return True

            @staticmethod
            def tree_entries(_sha):
                return {
                    "hallmark/LICENSE": (
                        "100644", "blob", "f4bfc53d8d773d3b10d95c2d58a5773dd4e2f3aa"
                    )
                }

        with mock.patch.object(cli.ancestry, "resolve",
                               return_value=(target, "main", None)), \
                mock.patch.object(cli.gitio, "fetch_commits",
                                  return_value=UnchangedWatchRepo()):
            result = cli._analyze_one(provider, root, tempfile.gettempdir())

        self.assertEqual(target, result.target_sha)
        self.assertTrue(result.drifted)
        self.assertTrue(result.is_blocked)
        self.assertEqual(states.BLOCKED, result.state)
        self.assertTrue(states.terminal(result.state))
        self.assertEqual([analyze.AUTHORITY], [reason.code for reason in result.blocked])
...
class TestArtifactOnlyCli(unittest.TestCase):
    def test_cli_has_no_publication_module_edge(self):
        self.assertNotIn("publish", cli.__dict__)
        self.assertNotIn("ghadapter", cli.__dict__)

    def test_prewrite_live_verification_is_structural(self):
        import inspect
        source = inspect.getsource(cli.cmd_prepare)
        final_verify = source.rfind("verify_workflow_environment")
        self.assertGreater(final_verify, source.find("_analyze_one("))
        self.assertLess(final_verify, source.find("candidate.create_artifact_root("))
        self.assertLess(final_verify, source.find("candidate.write(analysis"))

    def test_historical_publish_request_fails_as_uncommissioned(self):
        stderr = io.StringIO()
        with contextlib.redirect_stderr(stderr):
            result = cli.main(["prepare", "--publish"])
        self.assertEqual(result, 1)
        self.assertIn("UNCOMMISSIONED", stderr.getvalue())
