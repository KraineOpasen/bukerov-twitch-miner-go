...
* **Never the default branch.** Every push targets the complete stable-aware display locator
  returned by `candidate.branch_name()`. The base branch is explicit and only ever read.
...
This library is unit-testable but UNCOMMISSIONED and unreachable from the G1.1 production CLI
and workflow. Commit identity is passed per-invocation with `-c`, never written into repository
config, so a library test leaves no configuration residue in its checkout.
"""
...
COMMISSIONING_STATE = "UNCOMMISSIONED"
...
def _assert_publish_context(identity, base_branch):
    """Bind the testable publisher to the same complete stable identity as its artifact."""
    branch = branch_name(identity)
    if base_branch != identity.stable_branch:
        raise AdapterError(
            "publication base %r does not match candidate stable branch %r"
            % (base_branch, identity.stable_branch))
    return branch


def commit_candidate(repo_root, provider, analysis, identity, paths, base_branch):
    ...
    """
    branch = _assert_publish_context(identity, base_branch)
    ...
    body = (
        "%s\n\n"
        "Rendered by the UNCOMMISSIONED publication library for the stable-native artifact "
        "from reviewed ref %r.\nStable branch:  %s\nStable base:    %s\nProposal id:   "
        "%s\nSuperseded pin: %s\nTarget commit:   %s\n\n"
        ...
        % (PR_BANNER, provider.upstream_ref, identity.stable_branch,
           identity.stable_base_sha, identity.proposal_id, analysis.pinned_sha,
           analysis.target_sha))
    _git(repo_root, [
...
def publish_candidate(repo_root, provider, analysis, identity, paths, adapter, base_branch,
                      dry_run=False):
    ...
    if branch_present:
        ...
        return _open_pull_request(provider, analysis, identity, adapter, branch, base_branch,
                                  recovered=True)

    commit_candidate(repo_root, provider, analysis, identity, paths, base_branch)
    push_branch(repo_root, branch)
    return _open_pull_request(provider, analysis, identity, adapter, branch, base_branch)


def _open_pull_request(provider, analysis, identity, adapter, branch, base_branch,
                       recovered=False):
    ...
    try:
        pull = adapter.create_pull_request(
            ...
            body=report.pr_body(analysis, provider, identity), draft=True)
