...
class GitHubAdapter:
    ...
    def __init__(self, owner, repo, token=None, api_root=API_ROOT, timeout=30):
        self.owner = owner
        self.repo = repo
...
def from_env(env=None):
    """Build a real adapter from GITHUB_REPOSITORY / GITHUB_TOKEN."""
    env = env if env is not None else os.environ
    slug = env.get("GITHUB_REPOSITORY", "")
    if slug.count("/") != 1 or not all(slug.split("/")):
        raise AdapterError("GITHUB_REPOSITORY is not owner/repo: %r" % slug)
    owner, repo = slug.split("/")
    return GitHubAdapter(owner, repo, token=env.get("GITHUB_TOKEN"))
