...
POLICY_RELPATH = os.path.join("docs", "agents", "skills-maintenance", "policy.json")
CONTROL_PLANE_RELPATH = os.path.join(
...
def load(repo_root, path=None, *, require_stable_contract=True):
    ...
    """
    # The provider registry is no longer a self-sufficient control surface.  Production loads
    # must first prove the committed stable policy/control/schema envelope.  Fixture tests can
    # opt out explicitly; the production CLI never does.
    stable_contract = None
    if require_stable_contract:
        from . import runtime
        stable_contract = runtime.load_contract(repo_root)
        canonical_root = os.path.realpath(repo_root)
        canonical_path = os.path.abspath(os.path.join(canonical_root, CONFIG_RELPATH))
        if os.path.realpath(canonical_path) != canonical_path:
            raise ConfigError("canonical provider registry traverses a symlink")
        if path is not None:
            requested = path if os.path.isabs(path) else os.path.join(canonical_root, path)
            requested = os.path.abspath(requested)
            if requested != canonical_path or os.path.realpath(requested) != canonical_path:
                raise ConfigError(
                    "stable production may load only the committed provider registry %s"
                    % CONFIG_RELPATH)
        path = canonical_path
    else:
    ...
    for index, raw in enumerate(entries):
        ...
        provider.stable_contract = stable_contract
