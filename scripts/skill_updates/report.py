...
def to_json(analyses, plugin_drifts=None):
    ...
    payload = {
        ...
        "summary": {
            ...
            "ready": sum(1 for a in analyses if a.drifted and not a.is_blocked),
...
def text_report(analyses):
    ...
    lines.append("checked=%(checked)d drifted=%(drifted)d blocked=%(blocked)d ready=%(ready)d "
                 "discovery=%(discovery)d eval_required=%(eval)d" % summary)
    return "\n".join(lines) + "\n"


def eval_instructions(analysis, provider):
    ...
    lines = [
        ...
        "git switch %s               # the candidate" % (
            "automated/skills-update/%s-%s" % (provider.key, analysis.target_sha[:12])),
...
def pr_body(analysis, provider):
    ...
    parts = [
        PR_BANNER,
        "",
        "This branch was prepared mechanically by the scheduled skills-update bot",
        "(`.github/workflows/skills-update.yml`). **No audit has been performed.** The merge was",
        "clean and every blocked condition was checked, but \"merged without conflict\" is not",
        "\"reviewed\" -- only a human, or an agent under a task contract, can establish that.",
        "",
        "## What changed",
        "",
        "| field | value |",
        ...
        "",
        "The state is `%s`. Automation can reach that state and no other: `AUDITED` is"
        % analysis.state,
        "established only by a human (or an agent under a task contract) reading this diff and",
        "recording a fresh `reviewed_at`/`reviewed_by`. Only a fast-forward from the reviewed",
        ...
        "",
        "Each of these would have blocked the candidate and opened an issue instead:",
        "",
        ...
        "",
        "1. Read the actual content diff. Prose that merged cleanly can still change what a skill",
        "   instructs an agent to do -- that is a judgement call no checker makes.",
        "2. Re-read any script this provider ships if its bytes changed; `scripts_audited` in the",
        "   manifest is a claim about a human having read the file end to end.",
        "3. Confirm the local patches still say what they were written to say, in the new context.",
        "4. Update `reviewed_at` / `reviewed_by` and **delete the `automated_candidate` block**",
        "   from the manifest.",
        "",
        "`scripts/validate-agent-governance.py` fails while `automated_candidate` is present, so",
        "this candidate cannot pass the governance gate until step 4 is done deliberately.",
        "",
    ...
    if analysis.eval_required:
        parts += ["", eval_instructions(analysis, provider)]
    if analysis.discoveries:
        parts += ["", "## New sibling skills upstream (not installed)", "",
                  "These appeared upstream and were **not** installed; they are tracked in their",
                  "own `DISCOVERY_REQUIRED` issue and do not block this refresh:", ""]
...
def job_summary(analyses, mode):
    ...
    ready = [a for a in analyses if a.drifted and not a.is_blocked]
