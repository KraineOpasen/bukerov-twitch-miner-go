...
// transaction is released before the caller sees the value.
...
// method returns an error and a zero snapshot — never a partial one with a
// nil error — and the single connection is released, so the next write and
// read on the same handle proceed. database/sql rolls a cancelled
// transaction back on its own goroutine and refuses its commit, so the exact
// error is the context's or sql.ErrTxDone; only its presence is pinned.
func TestPointsSnapshotBetweenReleasesConnectionWhenCancelledMidRead(t *testing.T) {
	...
	rig.disarm()
	if err == nil {
		t.Fatalf("snapshot cancelled mid-read returned a nil error with %+v", snap)
	}
