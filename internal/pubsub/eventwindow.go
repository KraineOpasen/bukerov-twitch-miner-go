...
// Producers call mark() from arbitrary goroutines (reconnect handlers); the
// health watchdog calls count() once per minute. A plain atomic counter cannot
// express age-out, so a mutex-guarded ring is used: entries older than the
// query window are pruned on read, and the buffer is hard-capped so a window
// that is never read cannot grow without bound.
type eventWindow struct {
	mu    sync.Mutex
	times []time.Time
}

// eventWindowCap bounds retained timestamps. Reconnects are minutes apart, so
// any realistic window holds far fewer than this; the cap only guards against
