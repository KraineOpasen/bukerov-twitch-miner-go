...
// TestOrderedRulesDigestBindsTheReasonBehindAnAbsentValue pins the explanation
// as part of the evidence.
//
// Two candidates whose balance is absent for DIFFERENT reasons — one never
...
// TestOrderedRulesDigestsBindTheMandatoryFieldDeclarations closes the gap the
// mutation run found in the presence repair itself.
//
// Adding HasPosition, HasInterval and HasDefault made three previously
// unconditional fields refusable — and adding them to the digests without a
// test proving it is the same decoration mistake this suite already caught once
// for HasAvailableAtPosition. Deleting each digest line left the whole suite
// green, which is exactly the state in which a flag can be stripped from a
// stream the projection admitted while its digest still verifies.
//
// The assertion is on the DIGEST rather than on the refusal, deliberately: the
// evaluator re-establishes the projection's invariants too, so a stripped flag
// is refused either way and a status assertion would pass with the field
// unhashed. Only the digest value proves the field is bound.
func TestOrderedRulesDigestsBindTheMandatoryFieldDeclarations(t *testing.T) {
	cs := []predictioneval.OrderedRulesCandidate{
		orCandidate("c1", 10, orKnownBalance(1000), orOutcome("A", 4), orOutcome("B", 6)),
	}
	cfg := orConfig([]predictioneval.OrderedRule{
	...
	if base.Status != predictioneval.StatusWouldAttempt {
		t.Fatalf("premise: the intact input must evaluate; got %q", base.Status)
	}
	if base.StreamDigest == "" || base.ConfigDigest == "" {
		t.Fatal("premise: the intact result must carry both digests")
	}

	t.Run("candidate position declaration", func(t *testing.T) {
		s := orProject(t, cs, nil)
		s.Candidates[0].HasPosition = false
		got := predictioneval.EvaluateOrderedRules(s, cfg, orDraws())
		if got.StreamDigest == base.StreamDigest {
			t.Fatal("stripping HasPosition left the stream digest unchanged, so a projected stream " +
				"can lose its position declaration and still verify")
		}
	})

	t.Run("scope interval declaration", func(t *testing.T) {
		s := orProject(t, cs, nil)
		s.Scope.HasInterval = false
		got := predictioneval.EvaluateOrderedRules(s, cfg, orDraws())
		if got.StreamDigest == base.StreamDigest {
			t.Fatal("stripping HasInterval left the stream digest unchanged")
		}
	})
	...
	t.Run("config default declaration", func(t *testing.T) {
		...
		got := predictioneval.EvaluateOrderedRules(orProject(t, cs, nil), stripped, orDraws())
		switch {
		case got.ConfigDigest == base.ConfigDigest:
			t.Fatal("stripping HasDefault left the config digest unchanged, so a result cannot be " +
				"told apart from one computed against a config that really supplied its default")
		case got.Reason != predictioneval.ReasonConfigDefaultNotSupplied:
			t.Fatalf("reason = %q, want %q", got.Reason, predictioneval.ReasonConfigDefaultNotSupplied)
		}
	})
}
