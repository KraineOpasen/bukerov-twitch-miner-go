...
// quantities that still travel as numbers. That was correct, and a second
// reviewer then found the corrected list was itself short. The header
// enumerates those fields by name; this comment deliberately states no count,
// because a second copy of the figure is what went stale the first time — and
// the first attempt at this sentence put the number straight back in, which a
// third reviewer caught.
//
// This case is the evidence behind the header: it runs the real types through a
// consumer that has only float64 numbers and records what each path actually
// does. Two of the three input paths turn out to be DETECTED rather than
