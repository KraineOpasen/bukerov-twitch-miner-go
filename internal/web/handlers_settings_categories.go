...
// category page.
//
// Route 22 (System) still participates in the C6/C7/C8 dirty boundary like
// every other category page, but through a custom load/populate/gather/save
// adapter (see settings_system.html) instead of a JSON /api/settings
// payload: it drives the existing Health P3 canary/watchdog partial (GET
// /api/health, POST /api/health/settings) via that adapter, sequentially,
// so healthFormMu/ApplyHealthSettings semantics are preserved exactly —
// no new endpoint, DTO, or backend semantic either way.
//
