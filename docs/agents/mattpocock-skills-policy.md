...
What *is* wired into this repo is the G1.1 artifact-only detector/preparer at
`.github/workflows/stable-skills-maintenance.yml`. It can notice upstream drift and materialize an
audit-required artifact outside the checkout, but it cannot advance the repository pin. While `main` remains
the default branch and `release/0.3` remains non-default, this stable workflow is **UNCOMMISSIONED**. Even after
separate owner commissioning it can end only in `NO_DRIFT`, `BLOCKED`, or `PREPARED_AUDIT_REQUIRED`; it cannot
claim `AUDITED` or publish anything. See "Automated drift detection" below for the full contract.
