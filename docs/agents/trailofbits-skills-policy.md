...
- **Duplicates, subsets, or broken closures (6).** `trailmark-summary` and `trailmark-structural` are
  ...
  none; this stable line's `.github/workflows` holds `ci.yml`, `stable-release.yml`, and the G1.1
  `stable-skills-maintenance.yml` (which is **UNCOMMISSIONED** while `main` remains default). The historical
  default-main control plane separately carries `release.yml` and `skills-update.yml`. None invokes an AI
  coding action, so every run would terminate at step 2 with no output. That one is excluded for having no
  plausible use today, explicitly not for quality.
...
authority-surface key list (`docs/agents/skills-update-automation.md` on this stable line, where the G1.1
detector/preparer boundary is defined). A mechanical constraint sits
...
belongs to the skill (see `GOVERNANCE_V3.md` §10). Patch only for concrete project
...
methodology — agents, lanes, reviewers, writers, repair loops — and the project does not override it. See `GOVERNANCE_V3.md` §10, and the preserved topologies listed above.
