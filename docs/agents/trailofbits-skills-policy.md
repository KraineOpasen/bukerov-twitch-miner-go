...
- **Duplicates, subsets, or broken closures (6).** `trailmark-summary` and `trailmark-structural` are
  ...
  none; this stable line's `.github/workflows` holds `ci.yml` and `stable-release.yml` (the default
  branch additionally carries `release.yml` and `skills-update.yml`) and invokes no such action, so
...
authority-surface key list (`docs/agents/skills-update-automation.md` on the default branch, where the update automation is defined). A mechanical constraint sits
