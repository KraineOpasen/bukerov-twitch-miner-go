...
- Fixed stack [CP]: Go `html/template` (embedded via `go:embed` in `internal/web/server.go`) + Tailwind **v4** standalone CLI (`internal/web/static/css/input.css` → `app.css`, `make tailwind`; `@import "tailwindcss" source(none)` + `@source "../../templates"`) + HTMX (`static/js/htmx.min.js`) + ApexCharts (vendored) + i18n RU/EN (`t` template func) + SSE status stream (`/api/miner-status/stream`).
...
- **IA is frozen**: exactly **7 sections, 30 routes** (full list in §11). No route added, removed, or renamed; the routes-20/21 field-ownership correction in §11 is the sole owner-approved re-ownership exception — no further re-ownership.
...
1. 7 sections / 30 routes intact — none added, removed, or renamed; no re-ownership beyond the sole
   owner-approved routes-20/21 field-ownership correction in §11 — no further re-ownership relative to the
