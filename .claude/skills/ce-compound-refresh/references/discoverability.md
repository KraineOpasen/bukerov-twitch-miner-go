...
1. Find the project's root agent-instruction surface — `AGENTS.md`, `CLAUDE.md`, `GEMINI.md`, or whatever equivalent this project uses; the substantive file is the target, so ignore a shim that just `@`-includes another. <!-- bukerov-local-patch: ce-no-governance-file-writes -->In this repository `CLAUDE.md` and anything under `.claude/` are governance documents and are never edit targets — when the gap is there, report the suggested edit to the owner instead of editing.<!-- /bukerov-local-patch: ce-no-governance-file-writes --> No such file exists: skip this check.
