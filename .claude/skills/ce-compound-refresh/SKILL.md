...
Interactive: ask (per Blocking questions), with the recommended option first. <!-- bukerov-local-patch: ce-no-direct-main-push -->On the default branch: branch+commit+PR (recommended; specific branch name) / don't commit.<!-- /bukerov-local-patch: ce-no-direct-main-push --> On a clean feature branch: commit to it (recommended) / separate branch / don't commit. On a dirty feature branch: selective-stage and commit only refresh changes / don't commit.
...
1. Find the root instruction files (AGENTS.md, CLAUDE.md, or both); the substantive file is the target — ignore a shim that just `@`-includes the other. <!-- bukerov-local-patch: ce-no-governance-file-writes -->In this repository `CLAUDE.md` and anything under `.claude/` are governance documents and are never edit targets — when the gap is there, report the suggested edit to the owner instead of editing.<!-- /bukerov-local-patch: ce-no-governance-file-writes --> Neither exists: skip this check.
