...
After successful submit with ready (non-draft) PRs, continue to the babysit handoff below using the **bottom open non-draft** PR. <!-- bukerov-local-patch: ce-no-merge-authority -->Babysit posture is always `posture:stack-ready`.<!-- /bukerov-local-patch: ce-no-merge-authority --> Pass that posture on the `ce-babysit-pr` invocation (do not put `posture:` on this skill's argument-hint). Draft-only submit → hard residual before babysit when babysit is on.
...
```bash
gh pr create --draft --title "<TITLE>" --body-file "$BODY_FILE"   # new PR
