...
Every edit this file describes — the portable logic module, the task-runner script entry, folding the
validated reducer/machine into the real module — is still subject to [SKILL.md](SKILL.md)'s rule: prototypes
live in `/tmp` or a disposable worktree only, never the primary checkout, and no branch/commit/push happens
without an explicit task contract. The "lift into the real module" and "throwaway branch" steps need the
contract too.
<!-- /bukerov-local-patch: prototype-disposable -->
