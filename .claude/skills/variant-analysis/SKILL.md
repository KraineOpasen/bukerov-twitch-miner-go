...
<!-- bukerov-local-patch: tob-dead-pointer -->
...
Fan the five steps out over parallel Task subagents — one per expansion axis, looping until
the sweep stops finding anything new.
Each stage reads the reference above that matches its job.

Use the fan-out when the codebase is large or the root cause has many manifestations. Work
