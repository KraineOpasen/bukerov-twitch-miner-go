...
<!-- bukerov-local-patch: tob-effort-pin-dropped — upstream frontmatter carried `effort: low`, which Claude Code documents as overriding the session effort level while the skill is active; removed so the skill inherits the session's effort, and because this provider's frontmatter allowlist does not admit the key. See trailofbits-skills-patches.md. -->
