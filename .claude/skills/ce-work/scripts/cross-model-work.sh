...
adapter_argv() {
  case "$1" in
    codex)
      # bukerov-local-patch: ce-no-permission-bypass — dropped --ignore-user-config --ignore-rules; they discard the operator's own policy. --ephemeral and -s workspace-write stay.
      # Pin the editorial tier explicitly, matching the claude/grok routes' --effort high.
      printf '%s\0' codex exec --ephemeral \
    ...
    claude)
      ...
      # bukerov-local-patch: ce-no-permission-bypass — acceptEdits replaces bypassPermissions (the mode this script already uses for grok-cli); dropped the blanket --allowed-tools 'Bash(*)' grant, keeping the explicit --tools list.
      printf '%s\0' claude -p --safe-mode --no-session-persistence \
        --permission-mode acceptEdits --tools Read,Write,Edit,Bash \
    ...
    cursor)
      ...
      # bukerov-local-patch: ce-no-permission-bypass — dropped --force and --trust; --sandbox enabled stays.
      printf '%s\0' cursor-agent -p --output-format stream-json --stream-partial-output \
        --sandbox enabled --workspace "$WORKSPACE"
      [ "$cursor_model" = auto ] || printf '%s\0' --model "$cursor_model"
      ;;
    composer)
      # bukerov-local-patch: ce-no-permission-bypass — dropped --force and --trust; --sandbox enabled stays.
      printf '%s\0' cursor-agent -p --output-format stream-json --stream-partial-output \
        --sandbox enabled --workspace "$WORKSPACE" --model "$(route_model composer)"
      ;;
    grok-cursor)
      # bukerov-local-patch: ce-no-permission-bypass — dropped --force and --trust; --sandbox enabled stays.
      printf '%s\0' cursor-agent -p --output-format stream-json --stream-partial-output \
        --sandbox enabled --workspace "$WORKSPACE" --model "$(route_model grok-cursor)"
      ;;
    *) return 1 ;;
  esac
}
