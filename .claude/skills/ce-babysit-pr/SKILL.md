...
argument-hint: "[PR number|URL|blank=current branch] [watch|checkpoint] [duration] [posture:target|stack-ready]"
---
<!-- bukerov-local-patch: ce-no-merge-authority — dropped `stack-land` from the frontmatter argument-hint; the owner performs merges. -->
...
<!-- bukerov-local-patch: ce-no-merge-authority — removed the `stack-land` posture bullet (run-level land authorization, `gh stack merge` + `gh stack sync`); the owner performs merges. -->

One PR named → `target` (ask once if a confirmed multi-layer stack exists); own the stack → `stack-ready`.<!-- bukerov-local-patch: ce-no-merge-authority — deleted the "land → `stack-land`" selection clause. --> `mode:pipeline` never asks. Restate posture per transition.

## Non-negotiable boundaries

- <!-- bukerov-local-patch: ce-no-merge-authority -->**Merge-readiness is never merge authorization.** Under `target` and `stack-ready`, this skill never merges as part of babysitting.<!-- /bukerov-local-patch: ce-no-merge-authority -->
...
1. **Terminal check.** `MERGED`/`CLOSED` → stop.<!-- bukerov-local-patch: ce-no-merge-authority — deleted the `stack-land` exception; this run never lands a PR itself, so MERGED is always terminal. -->
...
5. **CI on the current head**, one pass for all failures: <!-- bukerov-local-patch: ce-no-workflow-rerun -->flaky/infra → extract the run ID and the host-qualified base repo from the failing check's `details_url` and surface it as a **rerun residual for the owner**; **never** run `gh run rerun` yourself<!-- /bukerov-local-patch: ce-no-workflow-rerun -->; real failure → `ce-debug mode:pipeline` once; mark each check acted on; unfixed checks stay red residuals.
