...
<!-- bukerov-local-patch: wizard-governance-gate -->
Explicit-invocation-only in this vendored copy — the agent never reaches for this skill on its own. Authoring
a wizard for infrastructure provisioning, credentials/CI secrets, third-party-dashboard setup, or a migration/
cutover still requires the active task contract to authorize that specific target operation; scope only what
the contract covers. The agent never runs the generated script itself, not even a single stage to "test" it —
...
instead; only the human-run script itself, not the agent, ever handles real secret values. This project's
`CLAUDE.md`, `.claude/rules/**`, and hooks always override this skill's instructions.
<!-- /bukerov-local-patch: wizard-governance-gate -->
