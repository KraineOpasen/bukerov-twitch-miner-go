...
- **A codebase, or more than one function** — <!-- bukerov-local-patch: tob-no-plugin-workflow -->orient over `<path>`, then
  analyze each function in its own `function-analyzer` subagent via Task, and write<!-- /bukerov-local-patch: tob-no-plugin-workflow --> `audit-context/DOSSIER.md` plus one file per
