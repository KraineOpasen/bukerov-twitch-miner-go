...
1. **The prose analysis**, written to the path you are given with the Write tool. This is the deliverable and
   it should be thorough. Follow <!-- bukerov-local-patch: tob-plugin-agents-relocated -->`{baseDir}/resources/ANALYSIS_FORMAT.md`<!-- /bukerov-local-patch: tob-plugin-agents-relocated -->.
...
- Worked examples, C and Solidity: `{baseDir}/resources/FUNCTION_MICRO_ANALYSIS_EXAMPLE.md`
- Per-domain mapping: `{baseDir}/resources/DOMAIN_NOTES.md` — read this when the
