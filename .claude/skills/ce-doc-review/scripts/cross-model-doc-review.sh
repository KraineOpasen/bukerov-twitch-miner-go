...
adapter_argv() {
  case "$1" in
    ...
    grok-cli)
      ...
      ;;
    grok-cursor)
      # bukerov-local-patch: ce-no-permission-bypass — dropped cursor-agent --trust (sandbox kept)
      printf '%s\0' cursor-agent -p --model "$(route_model grok-cursor)" --mode ask \
        --sandbox enabled --workspace "$PEER_WORKDIR" --output-format stream-json
      ;;
    cursor)
      # bukerov-local-patch: ce-no-permission-bypass — dropped cursor-agent --trust (sandbox kept)
      printf '%s\0' cursor-agent -p --mode ask \
        --sandbox enabled --workspace "$PEER_WORKDIR" --output-format stream-json
      ;;
    composer)
      # bukerov-local-patch: ce-no-permission-bypass — dropped cursor-agent --trust (sandbox kept)
      printf '%s\0' cursor-agent -p --model "$(route_model composer)" --mode ask \
        --sandbox enabled --workspace "$PEER_WORKDIR" --output-format stream-json
      ;;
    *) return 1 ;;
  esac
