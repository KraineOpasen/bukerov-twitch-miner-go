...
- **A project instruction file** (AGENTS.md or CLAUDE.md) — a small edit when the Discoverability Check finds a gap, **only in interactive Full mode after consent**. <!-- bukerov-local-patch: ce-no-governance-file-writes -->In this repository `CLAUDE.md` and anything under `.claude/` are governance documents and are never edit targets — report the suggested edit to the owner instead of applying it.<!-- /bukerov-local-patch: ce-no-governance-file-writes --> Non-interactive and lightweight never apply this edit (they report or tip instead). Callers that hand off non-interactive after an approval gate must not see an unreviewed change to the repo's operating contract.
