...
- **Phase 3 (workspace/branch):** Operate on the current branch — the orchestrator owns branch context; never prompt to create a branch, never prompt about uncommitted work. Commit the fix (`fix(ci): <summary>` for a CI failure, else `fix: <summary>`) and push. <!-- bukerov-local-patch: ce-no-direct-main-push -->Never commit or push while the current branch is the default branch (`main`/`master`) — in that case stop and report, leaving the fix uncommitted for the caller.<!-- /bukerov-local-patch: ce-no-direct-main-push --> Never weaken, skip, or mock a failing assertion to make it pass — repair the real issue or defer.
