...
<!-- bukerov-local-patch: ask-matt-setup-pointer -->
`/to-questionnaire` is not vendored in this project (excluded from the vendored skill set) — do not suggest it.
<!-- /bukerov-local-patch: ask-matt-setup-pointer -->
- **`/wizard`** — for the steps only a **human** can take: provisioning infrastructure, setting up credentials or CI secrets, clicking through an unfamiliar third-party dashboard, running a one-off migration or cutover. It generates an interactive bash script that opens each URL, captures each value, and writes it into `.env` and GitHub secrets — so the procedure stops being something you re-explain to an agent every time.
<!-- bukerov-local-patch: wizard-user-invoked -->
In this vendored copy it's explicit-invocation-only, not model-invoked: call `/wizard` yourself when you hit a
wall only a human can pass; the agent never reaches for it on its own, and any action it proposes still
requires the active task contract to authorize it.
<!-- /bukerov-local-patch: wizard-user-invoked -->
If the agent could just do it itself, it should; this is for where a human is genuinely in the loop.
- **`/wait-what`** — the corrective for a message that didn't land. Use it mid-conversation, inside any other skill, and the agent re-pitches what it just said with the context you were missing, in plain English, using
<!-- bukerov-local-patch: wait-what-domain-vocab -->
this project's domain/spec vocabulary (`CONTEXT.md`, `SPECIFICATIONS.md`)
<!-- /bukerov-local-patch: wait-what-domain-vocab -->
...
<!-- bukerov-local-patch: ask-matt-setup-pointer -->
The issue tracker, triage labels, and doc layout the other skills assume are already configured for this
project — see `docs/agents/issue-tracker.md`, `docs/agents/domain.md`, and `docs/agents/triage-labels.md`. Do
not suggest running `/setup-matt-pocock-skills` or `/to-questionnaire` (both excluded from this project's
vendored skill set).
<!-- /bukerov-local-patch: ask-matt-setup-pointer -->
