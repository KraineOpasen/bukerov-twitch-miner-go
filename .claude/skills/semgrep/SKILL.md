...
<!-- bukerov-local-patch: tob-no-plugin-workflow — dropped the frontmatter description's pointer to the /static-analysis:semgrep-scan plugin workflow, which does not exist in this project-local install -->
...
There is no plugin slash command in this project-local install — work the five steps below,
which are the same run with the ruleset selection visible and editable at Step 3.
<!-- /bukerov-local-patch: tob-no-plugin-workflow -->
