...
<!-- bukerov-local-patch: tob-no-tree-mutation -->
```bash
# Materialize the baseline commit in a temporary worktree; this does not move
# HEAD or modify the working tree. Scope paths below are relative to it.
git worktree add /tmp/baseline-review <baseline_commit>
...
<!-- bukerov-local-patch: tob-no-tree-mutation -->
After baseline analysis, remove the temporary worktree (`git worktree remove /tmp/baseline-review`) and analyze changes in the working tree.
<!-- /bukerov-local-patch: tob-no-tree-mutation -->
