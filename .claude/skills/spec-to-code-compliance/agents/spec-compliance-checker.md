...
1. **The analysis**, written with the Write tool to the path you are given. This is the deliverable. Follow
   <!-- bukerov-local-patch: tob-plugin-agents-relocated -->`{baseDir}/resources/ANALYSIS_FORMAT.md`<!-- /bukerov-local-patch: tob-plugin-agents-relocated -->.
...
- Output format: `{baseDir}/resources/ANALYSIS_FORMAT.md`
- Per-domain mapping: `{baseDir}/resources/DOMAIN_NOTES.md` — read this before
...
- Worked examples: `{baseDir}/resources/WORKED_EXAMPLE.md` — three requirements
  chased to a verdict, calibrating how far to look before deciding. Read when the verdict is not obvious.
- Severity: `{baseDir}/resources/DIVERGENCE_RUBRIC.md` — the line between a
