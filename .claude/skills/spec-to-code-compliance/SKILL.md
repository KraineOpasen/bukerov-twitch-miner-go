...
<!-- bukerov-local-patch: tob-no-plugin-workflow — frontmatter allowed-tools dropped `Workflow`: a project-local skill install has no plugin Workflow runtime. -->
...
<!-- bukerov-local-patch: tob-no-plugin-workflow -->
Dispatch one `spec-compliance-checker` subagent per requirement via Task, then a second, different agent to
refute each divergence. Fix the target path, the specification, and the fan-out before dispatching — "check
./contracts against SPEC.md, 20 requirements" — and pass those values into each subagent's prompt.

Find the documents, split them into individually checkable requirements, give each requirement its own
agent to hunt the code with, have independent agents try to refute every divergence before it is reported, and
write `spec-compliance/REPORT.md` plus one file per requirement under `spec-compliance/requirements/`. Only
...
Two properties come from <!-- bukerov-local-patch: tob-no-plugin-workflow -->the dispatch<!-- /bukerov-local-patch: tob-no-plugin-workflow --> rather than from instructions, and cannot be had here:

- **A refutation the finding's author did not perform.** Claude favors findings it produced when asked to check
  them. <!-- bukerov-local-patch: tob-no-plugin-workflow -->Send each divergence to agents that did not produce it — one reading the code again, one
  re-reading the document — and drop<!-- /bukerov-local-patch: tob-no-plugin-workflow --> what either knocks down.
...
it never — Δ +1.00. <!-- bukerov-local-patch: tob-no-plugin-workflow -->Installed here as a project-local skill there is no workflow command to carry
that on its own, so this section is the routing rather than a note about it: dispatch, and do not check
inline.<!-- /bukerov-local-patch: tob-no-plugin-workflow -->
