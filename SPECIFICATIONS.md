...
- **Per-operation client-ID fallback.** Generic GQL requests are tried against an ordered
  ...
  operation is tried first (cached in a mutex-guarded map, `opClientID`) — for BUSINESS reads; a diagnostic read caches nothing and therefore always starts from the shipped default — then
...
- **Minimum residence of the ordinary pair:** a complete configured Phase-A pair whose two members are both still valid candidates keeps its seats for a minimum of 15 minutes (`internal/watcher.fairRotationResidence`) before an *ordinary* persisted-deficit challenger may take one. This is a floor on how soon an ordinary swap can happen, never a switching cadence: reaching 15 minutes only re-opens ordinary reconciliation, which then keeps the same pair whenever the incumbents are still the two most owed. Nothing schedules a switch, and there is no goroutine, timer, store or setting behind it — the loop derives elapsed residence from `rotationState.lastSwitch`, the loop-owned anchor already published as `PairSince`, on the broker tick it already runs. The anchor is stamped only on initialization or an *unordered* membership change, so seat ordering, candidate permutation, and re-evaluating the same pair never refresh it; dropping below the slot cap or losing a member makes the stored anchor inert, and the next complete pair starts a fresh residence. Residence never delays a stronger cause, because it is only consulted when both incumbents are still candidates and only persisted deficit wants a different pair: a removed, offline, ineligible, avoided or otherwise invalid incumbent leaves at once, an empty seat refills at once, DROPS/STREAK and stronger-priority boosts apply afterwards in the same tick, failure/recovery and session invalidation are unaffected, and Phase-B/unified-broker arbitration still runs after Phase A. The bounded streak deferral below is only reachable once residence has expired — while the pair is still resident there is no replacement to defer, so its one-shot approach is neither armed nor consumed. Residence does apply to the ordinary ranking's own inputs (persisted deficit, the prefer handicap, and the recency/login tie-break), so the two halves of the per-streamer preference differ in latency: `avoid` removes a channel from the candidate set and takes effect on the next tick, while `prefer` only re-ranks and so takes effect at the next ordinary reconciliation. It likewise bounds how fast a boost latch's displaced victim rotates, because the base pair that victim is reselected against does not recompute while resident.
...
- **Bounded streak deferral:** when the current fair pair is about to lose an in-progress streak member, the loop may set one explicit `deferUntil = now + 2 min` for that pair approach. Re-evaluation cannot extend the deadline; expiry forces reconciliation, and a member that goes offline or loses protection leaves immediately. It is evaluated only after the 15-minute minimum residence above has expired. `PairSince` changes only when pair membership actually changes, and it is the same anchor the minimum residence is measured from. This best-effort protection does not cover imminent drop-campaign completion and never overrides a strictly stronger hard class.
