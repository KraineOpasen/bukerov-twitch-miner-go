...
- **Per-operation client-ID fallback.** Generic GQL requests are tried against an ordered
  ...
  operation is tried first (cached in a mutex-guarded map, `opClientID`) — for BUSINESS reads; a diagnostic read caches nothing per operation and therefore starts from the current process-wide default — the shipped one until a BUSINESS rotation promotes another — followed by the remaining shipped candidates; it never moves that default itself — then
