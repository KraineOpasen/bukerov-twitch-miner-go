...
`percentageUsers` exactly as the model held them), the points balance, the
global risk gates, and the original results of each stage. A `schedule_decision`
...
id needed to link a choice to its placement call; outcome titles, colours and
predictor data are not projected.
