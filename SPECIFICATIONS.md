...
- **Watch Streak milestone observability (diagnostic only):** an optional, read-only `RewardList` observation stage runs on the EXISTING bonus poll cycle (`internal/miner.bonusPollLoop`), strictly AFTER that cycle's business pass (bonus claiming and auto-redeem) has returned — never interleaved with it. It owns no ticker, goroutine, timer, cache, ledger, migration, public API, Statistics/UI field or support-bundle contract, and it adds no settings surface. At most **one** `RewardList` request per online streamer with a channel identity per bonus cycle; its only amplification is the shared read transport's existing client-ID fallback and transient-retry contract. Cancelling the loop context releases the in-flight request and abandons the remaining roster. Everything it produces is a structured diagnostic **log record** — retained logs are the ONLY persistence this feature has. Specifically:
  ...
  - **The official numeric ladder remains UNKNOWN.** `value`, `watchStreakThreshold`, `watchStreakCopoBonus`, `state` and milestone IDs are recorded as observed Twitch facts with no assigned meaning. There is no milestone-to-points truth table, and a received `+450` (or any other amount) implies **no** expected milestone tier. Every valid integer `WATCH_STREAK` amount — 300, 350, 400, 450 or otherwise — passes through the existing accounting path unchanged; none is filtered, rewritten, promoted or rejected for failing to match a presumed ladder.
  - **No historical reconstruction.** Past 300/350/400/450 milestones are not reconstructed, inferred or backfilled. Repeated identical milestone values observed at different times are distinct observations and are never deduplicated away; ordering is by request-start sequence, so an older request that completes later can never be presented as newer evidence. `achievementTimestamp` belongs to the observed achievement field and is recorded separately from the local request-start/request-end sampling window.
