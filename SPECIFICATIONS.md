...
|-------|--------------|--------|
| `community-points-user-v1` | `points-earned` | Update balance, log earnings, persist the accepted event to the exact point-event ledger (see *Exact Point-Event Ledger*); the frame's `point_gain` carries `total_points` (the accounted amount), `baseline_points` and `multipliers` (read only by the Watch Streak grant-correlation label, see *Watch Streak milestone observability*) |
