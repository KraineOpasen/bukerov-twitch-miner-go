...
| **Prototype / worktree** | `prototype` / `ce-prototype` (throwaway build answering a design question — `prototype` compares UI variants or builds one drivable logic/state demo, `ce-prototype` a narrow or wide set; `prototype` only in `/tmp` or a disposable worktree, `ce-prototype` under `.context/` with a `/tmp` fallback, its product-tree writes consented and never committed by it, and it alone needs a person present), `ce-worktree` (isolate work in a git worktree) |
...
| **Frontend / browser / design / a11y** | `frontend-design` (aesthetic direction for the Go html/template + Tailwind + HTMX + ApexCharts UI; visual only), `web-design-reviewer` (WCAG contrast, touch targets, focus order, reduced motion, viewport matrix), `webapp-testing` (drive the local dashboard, screenshots, console logs), `prototype` / `ce-prototype` (explore a surface before committing; `prototype` compares UI variants, `ce-prototype` narrow or wide sets) |
| **Durable knowledge / handoff** | `handoff` / `ce-handoff` (both create a handoff; `ce-handoff` also resumes one, `handoff` is explicit-invocation-only, and neither restores authority), `ce-compound` (solved problem → durable repo learning), `ce-compound-refresh` (audit learnings against current code), `harness-engineering` (repeated agent mistakes → durable instructions, drift checks, regression tests), `domain-modeling` (terminology/decisions that outlive the session), `writing-for-agents` (instructions another agent must follow), `teach` / `ce-explain` (`teach` is explicit-invocation-only, a multi-session teaching workspace; `ce-explain` builds at most one focused explainer per run and answers an operational question in chat) |
