...
| **Prototype / worktree** | `prototype` / `ce-prototype` (throwaway build answering a design question — `prototype` compares UI variants, `ce-prototype` narrow or wide sets; `prototype` only in `/tmp` or a disposable worktree, `ce-prototype` under `.context/` with a `/tmp` fallback, its product-tree writes consented and never committed by it, and only with a person present), `ce-worktree` (isolate work in a git worktree) |
