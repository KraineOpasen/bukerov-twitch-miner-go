...
| **Prototype / worktree** | `prototype` / `ce-prototype` (throwaway build answering a design question; both can compare alternatives — `prototype` only in `/tmp` or a disposable worktree, `ce-prototype` under `.context/` with a `/tmp` fallback, its overlay the one path touching the product tree and never committed, and only with a person present), `ce-worktree` (isolate work in a git worktree) |
